...
class PETADataset(Dataset):
    def __init__(self, root_dir: str, transform=None):
        ...
        self.num_total_attributes = (
            self.num_binary_attributes + self.num_color_attributes
        )

        self.binary_attr_map = {
            "accessoryHeadphone": 0,
            "personalLess15": 1,
            "personalLess30": 2,
            "personalLess45": 3,
            "personalLess60": 4,
            ...
            "lowerBodyFormal": 18,
            "upperBodyFormal": 19,
            "accessoryHairBand": 20,
            "accessoryHat": 21,
            "lowerBodyHotPants": 22,
            "upperBodyJacket": 23,
            "lowerBodyJeans": 24,
            "accessoryKerchief": 25,
            "footwearLeatherShoes": 26,
            "upperBodyLogo": 27,
            "hairLong": 28,
            "lowerBodyLongSkirt": 29,
            "upperBodyLongSleeve": 30,
            "lowerBodyPlaid": 31,
            "lowerBodyThinStripes": 32,
            "carryingLuggageCase": 33,
            "personalMale": 34,
            "carryingMessengerBag": 35,
            "accessoryMuffler": 36,
            "accessoryNothing": 37,
            "carryingNothing": 38,
            "upperBodyNoSleeve": 39,
            "upperBodyPlaid": 40,
            "carryingPlasticBags": 41,
            "footwearSandals": 42,
            "footwearShoes": 43,
            "hairShort": 44,
            "lowerBodyShorts": 45,
            "upperBodyShortSleeve": 46,
            "lowerBodyShortSkirt": 47,
            "footwearSneakers": 48,
            "footwearStocking": 49,
            "upperBodyThinStripes": 50,
            "upperBodySuit": 51,
            "carryingSuitcase": 52,
            "lowerBodySuits": 53,
            "accessorySunglasses": 54,
            "upperBodySweater": 55,
            "upperBodyThickStripes": 56,
            "lowerBodyTrousers": 57,
            "upperBodyTshirt": 58,
            "upperBodyOther": 59,
            "upperBodyVNeck": 60,
        }
...
class CNNMobileNet(nn.Module):
    def __init__(
        self,
        num_attributes: int = 105,
        is_utkface: bool = False,
        ...
        with torch.no_grad():
            ...
            if len(dummy_features.shape) == 4:
                ...
                raise ValueError(
                    f"Unexpected backbone output shape: {dummy_features.shape}"
                )
            in_features = dummy_features.shape[1]

        self.cnn_head = nn.Sequential(
            nn.Linear(in_features, 512), nn.ReLU(), nn.Dropout(dropout_rate)
        )
...
def train_peta(
    ...
    batch_size: int = 8,
    dropout_rate: float = 0.5,
    results_file: str = "hyperparameter_results.txt",
):
    ...
    for epoch in range(epochs):
        ...
        if (epoch + 1) % 5 == 0:
            metrics = evaluate_peta(
                model, val_loader, model_name, num_binary=61, num_colors=44
            )
...
def train_utkface(
    ...
    patience = 50
    counter = 0
    epsilon = 1e-8  # To avoid division by zero in dynamic weighting
    ...
    for epoch in range(epochs):
        ...
        for batch in train_loader:
            ...
            with autocast("cuda"): 
                ...
                w_age = 1.0 / (age_loss.item() + epsilon)
                w_gender = 1.0 / (gender_loss.item() + epsilon)
                w_race = 1.0 / (race_loss.item() + epsilon)
                total_w = w_age + w_gender + w_race
                w_age /= total_w
                w_gender /= total_w
                w_race /= total_w
                loss = w_age * age_loss + w_gender * gender_loss + w_race * race_loss
            scaler.scale(loss).backward()
            scaler.step(optimizer)
            scaler.update()
            train_loss += loss.item()
        ...
        with torch.no_grad():
            for batch in val_loader:
                ...
                with autocast("cuda"):  # Optional mixed precision
                    ...
                    race_loss = race_criterion(outputs["race"], labels["race"])
                    loss = (
                        0.1 * age_loss + gender_loss + race_loss
                    )  # Fixed weights for validation
                val_loss += loss.item()
        val_loss = val_loss / len(val_loader) if len(val_loader) > 0 else float("inf")
        print(f"Epoch {epoch+1}, Validation Loss: {val_loss}")
...
def evaluate_peta(
    model, loader, model_name: str, num_binary: int = 61, num_colors: int = 44
):
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    model.eval()
    preds, labels = [], []
    ...
    metrics = {"f1_binary": f1_score(binary_labels, binary_preds, average="micro")}
    colors = [
        "Black",
        "Blue",
        "Brown",
...
def evaluate_utkface(model, loader, model_name: str):
    ...
    with torch.no_grad():
        for batch in loader:
            ...
            outputs = model(images)
            age_errors.append(
                torch.abs(outputs["age"] - labels["age"].to(device)).cpu().numpy()
            )
    ...
    metrics = {"age_mae": age_mae, "gender_acc": gender_acc, "race_acc": race_acc}
    return metrics
...
def main():
    ...
    peta_train_dataset = PETADataset(
        peta_root, transform=get_transforms("PETA", is_training=True)
    )
    peta_val_dataset = PETADataset(
        peta_root, transform=get_transforms("PETA", is_training=False)
    )
    if len(peta_train_dataset) == 0:
        raise ValueError(
            "PETA dataset is empty. Check TownCentre folder and Label.txt."
        )

    utkface_train_dataset = UTKFaceDataset(
        utkface_root, transform=get_transforms("UTKFace", is_training=True)
    )
    utkface_val_dataset = UTKFaceDataset(
        utkface_root, transform=get_transforms("UTKFace", is_training=False)
    )
    ...
    mlcnn_configs = [
        ...
        {"name": "MLCNN_small", "num_conv_layers": 2, "hidden_units": 256},
    ]
    models_to_test = [
        (
            "CNN+MobileNet",
            CNNMobileNet,
            {"num_conv_layers": None, "hidden_units": None},
        ),
    ]
    ...
    for model_name, model_class, config in models_to_test:
        ...
        for lr, batch_size, dropout_rate, weight_decay, optimizer_type in product(
            ...
            utkface_train_loader = DataLoader(
                utkface_train_dataset,
    ...
    for model_name, model_class, config in models_to_test:
        ...
        for lr, batch_size, dropout_rate, weight_decay, optimizer_type in product(
            ...
            if model_name.startswith("MLCNN"):
                ...
                peta_model = model_class(
                    num_attributes=105, is_utkface=False, dropout_rate=dropout_rate
                )
